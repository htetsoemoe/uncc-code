const crypto = require("crypto");

console.log("------- Server ------- ");

const server = crypto.createDiffieHellmanGroup("modp15"); // 3072-bit key

/*
  Prime: 

  Generator: 0x02
*/

console.log("Prime Number: " + server.getPrime().toString("hex"));
console.log("\nGenerator: " + server.getGenerator().toString("hex"));

// Generating the server’s key pair
const serverPublicKey = server.generateKeys();
console.log("\nServer Public Key:", serverPublicKey.toString("hex"));

console.log("\n------- Client ------- ");

// The client has to load the same named group
const client = crypto.createDiffieHellmanGroup("modp15");

console.log("Prime Number: " + client.getPrime().toString("hex"));
console.log("\nGenerator: " + client.getGenerator().toString("hex"));

// Generating the client’s key pair
const clientPublicKey = client.generateKeys();
console.log("\nClient Public Key:", clientPublicKey.toString("hex"));

// ----- Key Exchange Process ----- //
console.log("\n------- Computed Secret ------- ");

const serverSecret = server.computeSecret(clientPublicKey);
const clientSecret = client.computeSecret(serverPublicKey);

console.log("Server Secret:", serverSecret.toString("hex"));
console.log("\nClient Secret:", clientSecret.toString("hex"));
